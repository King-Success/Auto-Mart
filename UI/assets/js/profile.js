    ...
    .then((res) => {
        ...
        if (res.status === 200) {
            ...
            if (data.length) {
                ...
                carsGrid.insertAdjacentHTML('beforeend', '<p>You currently have no advertisement to display</p>');
                return false
            }
        } else {
            carsGrid.insertAdjacentHTML('beforeend', '<p>Oops, looks like you haven\'t posted an advert yet')
        }
    })
    .catch((err) => {
        console.log(err);
