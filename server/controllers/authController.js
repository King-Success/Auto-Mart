...
class UserController {
  ...
  static async passwordResetEmail(req, res) {
    ...
    try {
      ...
      if (!user) return res.status(404).json({ status: 404, message: 'Invalid email, user not found' })
      const { id } = user
      const token = await generateToken({ email, id });
      const url = `${req.protocol}://${req.get('host')}/api/v1/users/reset/${token}`;
      ...
      return res.status(500).json({ status: 500, message: 'Internal server error', info });
    }
  }
  ...
  static async resetPassword(req, res) {
    ...
    if (!password || password.length < 6) {
      res.send(helper.resetTemplate(email, '<div class="alert">Password is must be at least 6 characters long</div>'));
